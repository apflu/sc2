...
房间要画成 region、工事点位要摆 marker、炮塔单位要建（按"单位由用户建"那条）。在这三样落地之前，代码侧写出来的框架跑不出任何可看的东西。

## 下一个

**第 12 项，防御设施 + 房间/走廊。**它挡着安保部任务 1 和控制部任务 1，协作杠杆 #4「代付防御升级」也全压在它上面，而且它是清单上唯一一个不依赖任何未决字段的。

**但如果编辑器侧还没动，先做第 7 项任务的计数一半。**它现在能数的东西比看起来多：
