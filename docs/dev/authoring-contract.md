...
`Regions` 是纯 XML，一条区域只有三样东西：

```xml
<region id="1">
    <name value="corridor_a"/>
    <shape type="rect">
        <quad value="35.9243,116.1076,56.4013,122.1354"/>
    </shape>
</region>
```

`quad` 是 `x0,y0,x1,y1`，整数也收。

> **千万别写 `<invisible/>`。**它是编辑器的"隐藏"开关，而一条被隐藏的区域**在编辑器里选不中、编不了、也看不见**。我照着两条已有区域抄了这一行，结果把全部八条区域一起变成了幽灵——地图上有、编辑器里没有。
>
> 教训比这一个字段大：**编辑器自己产出的对象是它的 schema 的唯一权威，而一个只有两个样本的字段不叫理解，叫巧合。**要抄，就抄一条刚由编辑器新建出来的。

区域名是**一条真通道**，代码按名字要东西：

| 前缀 | 谁在读 |
|---|---|
| `room_` / `corridor_` | `Fort_BuildRegistry` —— 只有这两个前缀会变成可升级的防御地点 |
| `zone_` | 部门分区的标线，见 [`../design/systems/ship.md`](../design/systems/ship.md)。**故意不在上面那两个前缀里**，所以不会污染地点表 |
| 其他（`resthall1`） | 各自的系统按名字取 |

**名字和 id 都不能重复。**同名＝一个按名字的查询有了两个答案；**同 id 则是编辑器那半边的事**——脚本按名字查所以毫无影响，但编辑器按 id 编号，撞了 id 的那条在编辑器里选不中。两样 `check_unique` 都拦。

**外缘不要画到镜头范围之外。**区域是靠点边框选中的，一条边落在镜头滚不到的地方就等于没有。

### 地形

**地形是可读的，不用描述给我听。**`python3 tools/show_terrain.py` 会把整张平面图打成字符画，`--at X Y --span N` 放大某一块。

能读出来的东西住在两个文件里，都不是那 9MB 的贴图遮罩：

| 文件 | 是什么 |
|---|---|
| `t3Terrain.xml` | 尺寸、地形集，还有一张 `<cliffCellList>`（**半分辨率**，96×96 覆盖 192×192） |
| `t3SyncCliffLevel` | `CLIF` + 32 字节头，然后每格一个 uint16。**值是悬崖等级 ×64**，所以 0 / 64 / 128 就是 0 / 1 / 2 级 |
| `t3CellFlags` | `LFCT` + 32 字节头，然后每格一字节。**第 0 位＝走不了** |

其余的（`t3TextureMasks`、`t3VertCol`、高度图）是涂装和微起伏，不改变游戏在什么上面进行。

**这张图是室内，墙就是悬崖。**所以"这两样东西中间有没有墙"是一个有精确答案的问题，答案就摆在文件里。
