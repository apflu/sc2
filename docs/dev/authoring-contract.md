...
一个异想体要三个单位类型，**靠共享的变体段绑定**（下面用 `Rabbit` 举例）：

| id | 是什么 | 从什么复制 |
|---|---|---|
| `Lob_Cont_Rabbit` | 收容单元，运作中 | 建筑（参考 `Lob_Device_Basic`） |
| `Lob_ContDown_Rabbit` | 收容单元，突破/被打坏 | 同上 + `Invulnerable`、`LifeStart=1` |
| `Lob_Abno_Rabbit` | 跑出来的异想体本身 | 战斗单位 |

前两个是一对普通的 downable（同 `Lob_Device_` ↔ `Lob_DeviceDown_`），第三个是**另一个对象**，不是第三种状态。

**工具异想体走另一个族**：`Lob_Tool_<X>` / `Lob_ToolDown_<X>`。不是 `Lob_Cont_Tool_<X>`——按前面那条"平凡的扫描要直接答出常见问题"，这里常见问题**排除**工具（融毁不数它、控制部跨部门条款单列它、培训兔兔是它），所以工具得落在 `Cont` 之外。要"含工具的全部单元"就扫两次，而且写出来一眼看得见。

各自要带上：

- **收容单元**：危险等级 attribute **恰好一个**（`Biological`/`Light`/`Armored`/`Psionic`/`User1` = Zayin/Teth/HE/WAW/Aleph）。归属玩家 = 负责它的那名玩家。
- **`Lob_ContDown_*`**：`Invulnerable`（虫群不会继续啃它，但 `Repair` 的过滤器不排除它，所以还能修）、`LifeStart=1`。
- **`Lob_Abno_*`**：不需要危险等级 attribute——等级读的是收容单元。归属玩家见下。

**还需要一个玩家槽。**跑出来的异想体要**同时**敌对虫群和全体玩家（设计 §5.1：放一个好处理的出来挡虫潮是合理战术）。这是地图玩家属性里的联盟设置，脚本补不了。代码侧用 `c_abnoPlayer = 3`；那个槽没配好之前，突破的异想体不会站在任何一边。

变体段没建 `Lob_Abno_` 的，突破时就只是"单元坏了但东西没出来"——对工具来说这正是想要的行为，也让半成品异想体不至于把地图搞崩。
