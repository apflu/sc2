...
**危险等级只是"突破后果"，不是"管理难度"。**这两件事在中段是反相关的：许多熟练玩家宁愿拿 WAW 也不愿意拿 HE。设计层的论证见 `docs/design/agents-and-abnormalities.md` §3，这里只定作者侧格式。

需要的第二个字段是**残余管理难度**——知道一切之后还剩多少麻烦。WAW 残余接近 0（属性够就锁得死），HE 残余高（强制损耗躲不掉）。

**它用剩下的 User 槽，机制和危险等级完全一样**：

| 字段 | Attribute | 含义 |
|---|---|---|
| 危险 tag | `User2` | 部门文档里控制部跨部门准入引用的那个 tag |
| 强制损耗 | `User3` | 残余管理难度 > 0：知识无法完全规避 |
| — | `User4` | 留空 |

用 attribute 而不是新建一套表，理由和危险等级那次一样，而且这次多一条：**它必须显示在单位面板上。**玩家在三选一界面里做的决定就依赖这个字段，而 attribute 是唯一自带面板显示、又能在文本编辑器里改显示名的通道。读取仍然是 `UnitTypeTestAttribute`，`06_risk.galaxy` 那套不用扩展。

代价照旧：**这两个槽从此也只有这一个含义**，别给它们配武器加成，否则会漏进伤害计算。

`User4` 明确留空——不要因为"还剩一个"就把某个想法塞进去。
